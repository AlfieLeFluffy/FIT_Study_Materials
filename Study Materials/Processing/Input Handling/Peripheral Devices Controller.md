...
The controller [[Register]]s, through which the [[Microprocessor|processor]] can communicate with the devices. These registers consist of **Data, Control and State** registers. 
## Device Mapping
Logically a device can be connected through two options:
- **Mapped IO** - The registers for controlling a device are mapped to the operation (main) [[Memory]] and the both the device and the system share an address space. The [[Microprocessor|processor]] can then work with the device as if it was just part of the memory.
- **Isolated IO** - The main memory and the device do not share an address space and both have their own space. Operations with this device are done through special [[Instruction|instruction]] such as IN, OUT or READ, WRITE.
## Physical Connection
