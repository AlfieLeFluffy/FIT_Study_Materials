...
As decimal numbers in the [[Binary Number System]] are represented with a sum of fractions it is not entirely possible to represent all decadic numbers on a finite amount of bits. For example the number 0.1 cannot be fully represented which is a common issue in financial software, which require higher precision and usually use different ways to represent numbers in different codes. For this they can use for example [[Binary Coded Decimal Code]].